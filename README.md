...
- 2021.07.06
  - [박소영] 회원가입 레이아웃 수정
  - [윤주현] 로그인 기능 활성화(form 방식, sha256 적용), Header에 로그인 정보 태우기, Header를 collapse 스타일로 변경, 메인 레이아웃 그리드형식으로 제작, Header 2단으로 분리 및 container 적용, 로그인 버튼 경계면 삭제로 너비 재조정, 로그아웃 시 발생하는 리다이렉트 문제 해결(Index -> /)
